...
def test_temp_to_stockhistory_migration(db_session, order_base_dependencies):
    ...
    migrated = migrate_temp_to_history(db_session, company_id, branch_id, stock_history_id=1)
    assert migrated >= 2
    # Verificar que existen detalles creados
    details = get_stock_history_details(db_session, company_id, branch_id, stock_history_id=1)
    assert len(details) >= 2
    # Borrar una entrada temporal residual (si quedó alguna no procesada)
    remaining = [t for t in get_tempstock_entries(db_session, company_id, branch_id) if t.IsProcessed == 0]
    for r in remaining:
        delete_tempstock_entry(db_session, company_id, branch_id, r.TempStockEntryID)
    # Afirmar que ya no quedan sin procesar
    assert all(t.IsProcessed == 1 for t in get_tempstock_entries(db_session, company_id, branch_id))