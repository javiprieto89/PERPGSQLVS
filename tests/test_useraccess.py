...
def test_create_get_update_delete_useraccess(db_session):
    ...
    max_role = db_session.execute(text("SELECT ISNULL(MAX(RoleID),0) FROM UserPermissions WHERE UserID=1 AND CompanyID=1 AND BranchID=1")).scalar() or 0
    ...
    assert any((o.UserID, o.CompanyID, o.BranchID, o.RoleID) == (obj.UserID, obj.CompanyID, obj.BranchID, obj.RoleID) for o in all_objs)
    # Actualizar
    update = UserAccessUpdate(BranchID=2)
    updated = update_useraccess(db_session, obj.UserID, obj.CompanyID, obj.BranchID, obj.RoleID, update)
    assert updated is not None and updated.BranchID == 2
    # Eliminar
    deleted = delete_useraccess(db_session, obj.UserID, obj.CompanyID, obj.BranchID, obj.RoleID)
    assert deleted is not None
    assert all((o.UserID, o.CompanyID, o.BranchID, o.RoleID) != (obj.UserID, obj.CompanyID, obj.BranchID, obj.RoleID) for o in get_useraccess(db_session))