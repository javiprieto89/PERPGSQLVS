# app/graphql/resolvers/sys/transactiontypes.py
import strawberry
from typing import List, Optional
from app.graphql.schemas.sys.transactiontypes import SysTransactionTypesInDB
...
class SystransactiontypesQuery:
    """Consultas para catálogo inmutable de tipos de transacción."""
    ...
    @strawberry.field
    def systransactiontypes_by_id(
        self, info: Info, id: int
    ) -> Optional[SysTransactionTypesInDB]:
        db_gen = get_db()
        db = next(db_gen)
        try:
            transactiontype = get_systransactiontypes_by_id(db, id)
            return (
                obj_to_schema(SysTransactionTypesInDB, transactiontype)
                if transactiontype
                else None
            )
        finally:
