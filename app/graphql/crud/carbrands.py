...
def get_carbrands_by_id(db: Session, company_id: int, carbrandid: int):
    return (
        db.query(CarBrands)
        .filter(CarBrands.CompanyID == company_id, CarBrands.CarBrandID == carbrandid)
        .first()
    )
...
def update_carbrands(db: Session, company_id: int, carbrandid: int, data: CarBrandsUpdate):
    obj = get_carbrands_by_id(db, company_id, carbrandid)
    if obj:
...
def delete_carbrands(db: Session, company_id: int, carbrandid: int):
    obj = get_carbrands_by_id(db, company_id, carbrandid)
    if obj:
        linked_models = (
            db.query(CarModels)
            .filter(CarModels.CompanyID == company_id, CarModels.CarBrandID == carbrandid)
            .first()
            is not None
        )
        if linked_models:
