...
class TempStockEntriesMutations:
    ...
    def update_tempstockentry(
        self, info: Info, id: int, data: TempStockEntriesUpdate
    ) -> Optional[TempStockEntriesInDB]:
        db_gen = get_db()
        db = next(db_gen)
        try:
            updated = update_tempstockentries(db, id, data)
            return obj_to_schema(TempStockEntriesInDB, updated) if updated else None
        finally:
            db_gen.close()

    @strawberry.mutation
    def delete_tempstockentry(self, info: Info, id: int) -> bool:
        db_gen = get_db()
        db = next(db_gen)
        try:
            deleted = delete_tempstockentries(db, id)
    ...
    def process_stock_session(
        ...
        db = next(db_gen)
        try:
            history = process_tempstockentries(db, sessionID)
            return list_to_schema(StockHistoryInDB, history)
        finally:
            db_gen.close()
