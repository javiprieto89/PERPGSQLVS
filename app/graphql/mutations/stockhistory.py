...
class StockHistoryMutations:
    ...
    def update_stockhistory(
        self, info: Info, id: int, data: StockHistoryUpdate
    ) -> Optional[StockHistoryInDB]:
        db_gen = get_db()
        db = next(db_gen)
        try:
            updated = update_stockhistory(db, id, data)
            return obj_to_schema(StockHistoryInDB, updated) if updated else None
        finally:
            db_gen.close()

    @strawberry.mutation
    def delete_stockhistory(self, info: Info, id: int) -> bool:
        db_gen = get_db()
        db = next(db_gen)
        try:
            deleted = delete_stockhistory(db, id)
