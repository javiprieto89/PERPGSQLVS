...
class BankAccounts(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(['CurrencyID'], [
                             'sysCurrencies.CurrencyID'], name='FK_BankAccounts_Currencies'),
        ForeignKeyConstraint(['CompanyID'], ['Company.CompanyID'],
                             name='FK_BankAccounts_Company'),
        PrimaryKeyConstraint(
            'BankAccountID', name='PK__BankAcco__4FC8E741DC7098A4'),
        Index('IX_FK_FK_BankAccounts_Banks', 'BankID'),
        Index('IX_FK_FK_BankAccounts_Currencies', 'CurrencyID'),
        Index('UQ__BankAcco__D675B09BF181EA6F',
              'CompanyID', 'AccountNumber', unique=True)
    )
    ...
    Alias: Mapped[Optional[str]] = mapped_column(
        String(80, 'Modern_Spanish_CI_AS'))

    company_: Mapped['Company'] = relationship(
        'Company',
        back_populates='bankaccounts',
        primaryjoin='BankAccounts.CompanyID == Company.CompanyID',
        foreign_keys='[BankAccounts.CompanyID]',
        overlaps='bankaccounts'
    )

    Banks_: Mapped['Banks'] = relationship(
        'Banks', back_populates='bankaccounts')
    sysCurrencies: Mapped['SysCurrencies'] = relationship(
        'SysCurrencies', back_populates='BankAccounts')
    BankReconciliations: Mapped[List['BankReconciliations']] = relationship(
        'BankReconciliations',
        back_populates='BankAccounts_',
        overlaps='Company_'
    )
    BankTransactions: Mapped[List['BankTransactions']] = relationship(
        'BankTransactions', back_populates='BankAccounts_')
    CheckMovements: Mapped[List['CheckMovements']] = relationship(
        'CheckMovements', back_populates='BankAccounts_')
