...
class Checks(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['BankID'], ['Banks.BankID'], name='FK_Checks_Banks'),
        ForeignKeyConstraint(['CheckStatusID'], ['CheckStatuses.CheckStatusID'], name='FK_Checks_CheckStatuses'),
        ForeignKeyConstraint(['CurrencyID'], ['sysCurrencies.CurrencyID'], name='FK_Checks_Currencies'),
    ...
    CheckID: Mapped[int] = mapped_column(BigInteger, Identity(start=1, increment=1), primary_key=True)
    ...
    StatusAt: Mapped[datetime.datetime] = mapped_column(DATETIME2, server_default=text('(sysutcdatetime())'))
    ...
    DrawerName: Mapped[Optional[str]] = mapped_column(Unicode(120, 'Modern_Spanish_CI_AS'))
    HolderName: Mapped[Optional[str]] = mapped_column(Unicode(120, 'Modern_Spanish_CI_AS'))
    ...
    CheckStatuses_: Mapped[Optional['CheckStatuses']] = relationship('CheckStatuses', back_populates='Checks')
    sysCurrencies: Mapped['SysCurrencies'] = relationship('SysCurrencies', back_populates='Checks')
    CheckMovements: Mapped[List['CheckMovements']] = relationship('CheckMovements', back_populates='Checks_')
