...
class Warehouses(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            'WarehouseID', name='PK__Warehous__2608AFD9216E9B31'),
    )

    WarehouseID = Column(Integer, Identity(
    ...
    orderDetails_: Mapped[List['OrderDetails']] = relationship(
        'OrderDetails', back_populates='warehouses_')
    tempOrderDetails: Mapped[List['TempOrderDetails']] = relationship(
        'TempOrderDetails', back_populates='warehouses_')
    orderHistoryDetails: Mapped[List['OrderHistoryDetails']] = relationship(
        'OrderHistoryDetails', back_populates='warehouses_')
