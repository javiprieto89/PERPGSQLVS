...
class CashBoxes(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(["CompanyID", "BranchID"], [
                             "Branches.CompanyID", "Branches.BranchID"], name="FK_CashBoxes_CompanyBranch"),
        ForeignKeyConstraint(["UserID"], ["Users.UserID"],
                             name="FK_CashBoxes_User"),
        PrimaryKeyConstraint("CashBoxID", name="PK_CashBoxes"),
    )

    CashBoxID = Column(Integer, Identity(
    ...
    branches_: Mapped[Optional['Branches']] = relationship(
        "Branches", back_populates="cashBoxes")
    users_: Mapped[Optional['Users']] = relationship(
        "Users", back_populates="cashBoxes")
    cashBoxMovements: Mapped[List['CashBoxMovements']] = relationship(
        "CashBoxMovements", back_populates="cashBoxes_")
