...
class OrderDetails(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['ItemID'], ['Items.ItemID'],
                             name='FK__OrderDeta__ItemI__2DE6D218'),
        ForeignKeyConstraint(['OrderID'], ['Orders.OrderID'],
                             name='FK__OrderDeta__Order__2CF2ADDF'),
        ForeignKeyConstraint(['WarehouseID'], [
                             'Warehouses.WarehouseID'], name='FK_OrderDetails_Warehouses'),
        PrimaryKeyConstraint(
            'OrderDetailID', name='PK__OrderDet__9DD74D9DF5D37EDA')
    )

    OrderDetailID = Column(Integer, Identity(
    ...
    items_: Mapped['Items'] = relationship(
        'Items', back_populates='orderDetails_')
    orders_: Mapped['Orders'] = relationship(
        'Orders', back_populates='orderDetails_')
    warehouses_: Mapped['Warehouses'] = relationship(
        'Warehouses', back_populates='orderDetails_')
