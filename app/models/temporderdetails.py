...
class TempOrderDetails(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(['CompanyID'], [
                             'CompanyData.CompanyID'], name='FK__TempOrder__Compa__0C85DE4D'),
        ForeignKeyConstraint(['ItemID'], ['Items.ItemID'],
                             name='FK__TempOrder__ItemI__0F624AF8'),
        ForeignKeyConstraint(['OrderID'], ['Orders.OrderID'],
                             name='FK_TempOrderDetails_Orders'),
        ForeignKeyConstraint(['OrderDetailID'], [
                             'OrderDetails.OrderDetailID'], name='FK_TempOrderDetails_OrderDetails'),
        ForeignKeyConstraint(['PriceListID'], [
                             'PriceLists.PriceListID'], name='FK_TempOrderDetails_PriceLists'),
        ForeignKeyConstraint(['UserID'], ['Users.UserID'],
                             name='FK__TempOrder__UserI__0E6E26BF'),
        ForeignKeyConstraint(['WarehouseID'], [
                             'Warehouses.WarehouseID'], name='FK_TempOrderDetails_Warehouses'),
    ...
    OrderSessionID = Column(
        Uuid, server_default=text('(newid())'), nullable=False)
    ...
    orders_: Mapped[Optional['Orders']] = relationship(
        'Orders', back_populates='tempOrderDetails')
    priceLists_: Mapped['PriceLists'] = relationship(
        'PriceLists', back_populates='tempOrderDetails')
    users_: Mapped['Users'] = relationship(
        'Users', back_populates='tempOrderDetails')
    warehouses_: Mapped['Warehouses'] = relationship(
        'Warehouses', back_populates='tempOrderDetails')
