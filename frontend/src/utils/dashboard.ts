import { type GetDashboardDataQuery } from "~/graphql/_generated/graphql";

// frontend/src/graphql/dashboard.js
export const dashboardHelpers = {
  processDashboardData(data: GetDashboardDataQuery, companyId?: number) {
    ...
    const lowStockItems = filteredStock.filter((stock) => {
      ...
      return (
        ...
        item.ControlStock &&
        (stock.Quantity || 0) <=
          (stock.MinStockLevel || item.ReplenishmentStock || 0) &&
        (stock.Quantity || 0) > 0
      );
    });
    ...
    return {
      ...
      pendingOrders: filteredOrders.filter((o) =>
        [1, 2].includes(o.OrderStatusID)
      ).length,
      completedOrders: filteredOrders.filter((o) => o.OrderStatusID === 3)
        .length,
