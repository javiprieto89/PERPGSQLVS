...
export default function CarBrands() {
  ...
  const allData = data?.allCarbrands || [];
  ...
  const handleEdit = useCallback(
    (row: DataInDB) => navigate(`form/${row.CarBrandID}`),
    [navigate]
  );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      ...
      try {
        await carBrandOperations.deleteCarBrand(String(id));
        refetch();
      } catch (err) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TableActionButton
            row={row}
            onDelete={() => handleDelete(row.original.CarBrandID)}
  ...
  return (
    <section className="section">
      <AdminTopBar title="Marcas de Vehículos" quickAccessHidden>
