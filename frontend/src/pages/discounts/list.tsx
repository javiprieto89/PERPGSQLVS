...
import { useGetAllDiscountsQuery, type DiscountsInDb } from "~/graphql/_generated/graphql";
import { discountOperations } from "~/services/discount.service";

type DataInDB = DiscountsInDb;
...
export default function Discounts() {
  ...
  const allData = data?.allDiscounts || [];
  ...
  const handleEdit = useCallback(
    (row: DataInDB) => navigate(`form/${row.DiscountID}`),
    [navigate]
  );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      ...
      try {
        await discountOperations.deleteDiscount(String(id));
        refetch();
      } catch (err) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TableActionButton
            row={row}
            onDelete={() => handleDelete(row.original.DiscountID)}
  ...
  return (
    <section className="section">
      <AdminTopBar title="Descuentos" quickAccessHidden>
