...
import SaleConditionSearchModal from "~/features/salecondition/SaleConditionSearchModal";
...
export function Orderform({
  ...
  useEffect(() => {
    if (initialOrder) {
      ...
      setFormData({
        ...
        serviceTypeId: initialOrder.ServiceTypeID
          ? String(initialOrder.ServiceTypeID)
          : "",
        mileage: initialOrder.Mileage ? String(initialOrder.Mileage) : "",
        nextServiceMileage: initialOrder.NextServiceMileage
          ? String(initialOrder.NextServiceMileage)
          : "",
  ...
  const handleSubmit = async (e) => {
    ...
    const orderData = {
      ...
      ServiceTypeID: finalFormData.serviceTypeId
        ? parseInt(finalFormData.serviceTypeId)
        : null,
      Mileage: finalFormData.mileage ? parseInt(finalFormData.mileage) : null,
      NextServiceMileage: finalFormData.nextServiceMileage
        ? parseInt(finalFormData.nextServiceMileage)
        : null,
    ...
    try {
      ...
      if (isEdit && initialOrder?.OrderID) {
        response = await orderOperations.updateOrder(
          initialOrder.OrderID,
          orderData
        );
      } else {
  ...
  useEffect(() => {
    const cleanup = () => {
      if (sessionId) {
        tempOrderOperations.clearTempSession(sessionId, true).catch((err) => {
          console.error("Error limpiando items temporales:", err);
        });
      }
    };
    ...
    <div className="container mx-auto p-4 md:p-6  min-h-screen">
      ...
      {showSaleConditionModal && (
        <SaleConditionSearchModal
          ...
          onSelect={(sc) => {
            setFormData((prev) => ({
              ...prev,
              saleConditionId: sc.SaleConditionID,
            }));
      ...
      {showItemConfirmationModal && selectedItemForConfirmation && (
        <ItemConfirmationModal
          ...
          confirmLabel={
            editIndex !== null ? "Actualizar Ítem" : "Agregar al Pedido"
          }
          priceLists={priceLists}
          warehouses={warehouses}
          defaultPriceListId={
            selectedItemForConfirmation.priceListId || formData.priceListId
          }
          defaultWarehouseId={
            selectedItemForConfirmation.warehouseId || formData.warehouseId
          }
        />
      )}
    </div>
