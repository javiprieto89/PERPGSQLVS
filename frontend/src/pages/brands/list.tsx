...
import { useGetAllBrandsQuery, type BrandsInDb } from "~/graphql/_generated/graphql";
import { brandOperations } from "~/services/brand.service";

type DataInDB = BrandsInDb;
...
export default function Brands() {
  ...
  const handleEdit = useCallback(
    (row: DataInDB) => navigate(`form/${row.BrandID}`),
    [navigate]
  );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      ...
      try {
        await brandOperations.deleteBrand(String(id));
        refetch();
      } catch (err) {
        alert("Error al borrar marca: " + (err as Error).message);
      }
    },
    [refetch]
  );
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TableActionButton
            row={row}
            onDelete={() => handleDelete(row.original.BrandID)}
