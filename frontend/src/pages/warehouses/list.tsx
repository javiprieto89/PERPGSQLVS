...
export default function Warehouses() {
  ...
  const allData = data?.allWarehouses || [];
  ...
  const handleEdit = useCallback(
    (row: DataInDB) => navigate(`form/${row.WarehouseID}`),
    [navigate]
  );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      ...
      try {
        await warehouseOperations.deleteWarehouse(String(id));
        refetch();
      } catch (err) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TableActionButton
            row={row}
            onDelete={() => handleDelete(row.original.WarehouseID)}
  ...
  return (
    <section className="section">
      <AdminTopBar title="Almacenes" quickAccessHidden>
