...
import { useGetAllServicetypesQuery, type ServiceTypeInDb } from "~/graphql/_generated/graphql";
import { serviceTypeOperations } from "~/services/service-type.service";

type DataInDB = ServiceTypeInDb;
...
export default function ServiceTypes() {
  ...
  const allData = data?.allServicetypes || [];
  ...
  const handleEdit = useCallback(
    (row: DataInDB) => navigate(`form/${row.ServiceTypeID}`),
    [navigate]
  );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      ...
      try {
        await serviceTypeOperations.deleteServiceType(String(id));
        refetch();
      } catch (err) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TableActionButton
            row={row}
            onDelete={() => handleDelete(row.original.ServiceTypeID)}
  ...
  return (
    <section className="section">
      <AdminTopBar title="Tipos de Servicio" quickAccessHidden>
