...
export default function Roles() {
  ...
  const handleEdit = useCallback(
    (row: DataInDB) => navigate(`form/${row.RoleID}`),
    [navigate]
  );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      ...
      try {
        await roleOperations.deleteRole(String(id));
        refetch();
      } catch (err) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TableActionButton
            row={row}
            onDelete={() => handleDelete(row.original.RoleID)}
