...
import { useGetPriceListsQuery, type PriceListsInDb } from "~/graphql/_generated/graphql";
import { pricelistOperations } from "~/services/price-list.service";

type DataInDB = PriceListsInDb;
...
export default function PriceLists() {
  ...
  const allData = data?.allPricelists || [];
  ...
  const handleEdit = useCallback(
    (row: DataInDB) => navigate(`form/${row.PriceListID}`),
    [navigate]
  );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      ...
      try {
        await pricelistOperations.deletePricelist(String(id));
        refetch();
      } catch (err) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <TableActionButton
            row={row}
            onDelete={() => handleDelete(row.original.PriceListID)}
