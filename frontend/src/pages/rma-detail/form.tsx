...
import { BASE_ROUTE, useRmaDetailForm } from "~/features/rma-detail/useRmaDetailForm";

export function RmaDetailForm() {
  const params = useParams();
  const id = params.id ? Number(params.id) : undefined;

  const { form, handleSubmit, data, isEditing, isLoading, isSaving, errors } = useRmaDetailForm({ id });
    ...
    <>
      ...
      <AdminTopBar>
        <FormBreadcrumb isEditing={isEditing}>
          <Link to={BASE_ROUTE}>Detalle de RMA</Link>
        </FormBreadcrumb>
      </AdminTopBar>
      ...
      <div className="container p-6">
        <div className="flex justify-between items-center gap-2 mb-6 md:max-w-[700px] lg:max-w-[800px]">
          <h2 className="font-xl font-bold">
            {isEditing ? "Editar Detalle de RMA" : "Nuevo Detalle de RMA"}
          </h2>
          {data?.RmaDetailID && (
            <div className="flex gap-4 text-muted-foreground text-sm">
              <span className="">ID: {data?.RmaDetailID}</span>
              <Separator orientation="vertical"
                className="mx-2 data-[orientation=vertical]:h-4"
              />
              <span className="text-sm">
                <strong>RMA:</strong> {data.RmaID}
              </span>
            </div>
          )}
        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormBlock>
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="space-y-2">
                              <FormLabel>RMA*</FormLabel>
                              <FormControl>
                                <Input
                                  {...field}
                                  type="number"
                                  placeholder="ID del RMA"
                                  onChange={(e) => field.onChange(Number(e.target.value))}
                                  value={field.value?.toString() || ""}
                                />
                              </FormControl>
                    ...
                    <FormBlock>
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="space-y-2">
                            <FormLabel>Descripción de Línea</FormLabel>
                            <FormControl>
                              <Textarea
                                {...field}
                                value={field.value || ""}
