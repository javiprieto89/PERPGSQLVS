...
import { bankReconciliationService, type BankReconciliation } from "~/services/bank-reconciliation.service";
import { AuthHelper } from "~/utils/authHelper";
import { bankReconciliationHelpers, formSchema, type FormSchema } from "./bankReconciliationHelpers";
...
export function useBankReconciliationForm({ id }: UseBankReconciliationFormOptions = {}) {
    ...
    bankReconciliationService
      ...
      .then((reconciliation) => {
        if (reconciliation) {
          ...
          form.reset(bankReconciliationHelpers.prepareDataFormSchema(reconciliation), {
            keepDirty: true,
  ...
  async function handleSubmit(formData: FormSchema) {
    ...
    try {
      ...
      if (isEditing && id) {
        result = await bankReconciliationService.update(id, preparedData, formData.CompanyID);
      } else {
      ...
      toast.error("Error al guardar la conciliación bancaria. Contacte al administrador del sistema");
    } finally {
      setIsSaving(false);
    }
  }
