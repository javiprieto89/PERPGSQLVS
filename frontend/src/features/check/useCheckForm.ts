...
export function useCheckForm({ id }: UseCheckFormOptions = {}) {
  ...
  async function handleSubmit(formData: FormSchema) {
    ...
    try {
      ...
      if (isEditing && id) {
        result = await checkService.update(
          id,
          preparedData,
          formData.CompanyID
        );
      ...
      toast.message(
        isEditing ? "Cheque actualizado con éxito" : "Cheque creado con éxito"
      );
    } catch (error) {
      setSaveError(error as Error);
      console.error("Error saving check:", error);
      toast.error(
        "Error al guardar el cheque. Contacte al administrador del sistema"
      );
    } finally {
      setIsSaving(false);
    }
