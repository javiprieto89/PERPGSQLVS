...
import { useClientForm } from "~/features/client/useClientForm";
...
export function ClientForm({
  ...
  const { form, handleSubmit, data, isEditing, isLoading, isSaving, errors } = useClientForm({ id, onSave });
  const { data: formData, loading: formDataLoading } = useGetClientFormDataQuery();

  return (
    <div>
      {import.meta.env.DEV && <DevTool control={form.control} />}
      <div className="flex justify-between items-center gap-2 mb-6 md:max-w-[700px]">
        <h2 className="font-xl font-bold">
          {isEditing ? "Editar Cliente" : "Nuevo Cliente"}
        </h2>
        {data?.ClientID && (
          <div className="flex gap-4 text-muted-foreground text-sm">
            <span>ID: {data?.ClientID}</span>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4 md:max-w-[700px]">
                        ...
                        <FormItem className="space-y-2">
                          ...
                          <FormControl>
                            <SelectWithSearch
                              ...
                              accessor="DocTypeID"
                              display="DocTypeName"
                              data={formData?.sysIdentityDocTypes || []}
                  ...
                  <FormBlock>
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={formDataLoading ? "Loading..." : formData?.allCompany.length === 0 ? "Sin opciones" : "Seleccione..."} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectGroup>
                  ...
                  <FormBlock>
                    ...
                    <FormField
                      ...
                      render={({ field, fieldState }) => {
                        ...
                        const availableProvinces = (formData?.allProvinces || []).filter(
                ...
                <div className="md:flex space-x-4">
                  <FormBlock>
                    <FormSelect
                      ...
                      options={formData?.allPricelists}
                      loading={formDataLoading}
