...
MODELS_DIR = r'C:\Users\javie\OneDrive\_Development\PERPGSQLVS\app\models'  # <--- CAMBIA ESTA RUTA
BACKUP_DIR = fr'S:\back_modelos_{datetime.datetime.now():%Y%m%d_%H%M%S}'
...
def find_relationships(content):
    regex = re.compile(
        r"(\w+)\s*:\s*Mapped.*?=\s*relationship\(\s*['\"](\w+)['\"].*?back_populates\s*=\s*['\"](\w+)['\"]", re.S)
...
def replace_relationship(content, attr, modelo, backpop):
    pattern = re.compile(
        rf"{attr}\s*:\s*Mapped.*?=\s*relationship\(['\"]{modelo}['\"].*?back_populates\s*=\s*['\"].*?['\"].*?\)", re.S)
...
def add_relationship(content, attr, modelo, backpop):
    ...
    for i in range(len(lines)-1, -1, -1):
        if lines[i].strip().startswith('# Relaciones'):
            lines.insert(
                i+1, f"    {attr}: Mapped = relationship('{modelo}', back_populates='{backpop}')")
            break
        if lines[i].startswith('class '):
            lines.insert(
                i+2, f"    {attr}: Mapped = relationship('{modelo}', back_populates='{backpop}')")
            break
    return "\n".join(lines)
    ...
    for r2 in relaciones:
        if r2['clase'] == modelo and r2['modelo'] == clase:
            if r2['backpop'] != attr:
                ...
                nuevo = replace_relationship(
                    contenido, r2['attr'], clase, attr)
            ...
            archivo_modelo = os.path.join(
                MODELS_DIR, modelos[modelo]['archivo'])
